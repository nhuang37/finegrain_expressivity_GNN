...
from gnn_baselines.gnn_architectures import GIN, GINE, GC, GC_meanpool, GIN_meanpool
import os
import argparse
import numpy as np
import pickle
...
def main(args):
    ...
    for d, use_labels in dataset:
        ...
        acc, s_1, s_2, time, t_std = gnn_evaluation(GIN_meanpool, d, [args.layer], [args.hid_dim], max_num_epochs=200, batch_size=64,
                                       start_lr=0.01, num_repetitions=args.num_reps, all_std=True, untrain=False)
        ...
        file_name = os.path.join(args.result_path, d + "_" + str(args.hid_dim) 
                                 + "_" + str(args.layer) + "_timed_constant_mean.pkl")
