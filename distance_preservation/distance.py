...
import time
import scipy 
#note numeric issue of wassersteinLP due to ill-conditioning for scipy 1.7.3, 
#works fine for scipy 1.10.0 (defaulte method = 'high' instead of 'interior-point')
#ref:https://docs.scipy.org/doc/scipy/reference/optimize.linprog-interior-point.html
    ...
    while not converged and it < iterationBound and convIt < convIterationBound:
        ...
        for v in I:
            ...
            for u in I.neighbors(v):
                color[colorOf[u]] += otherGraphSize #e.g., iter 1: count degree (normalize by graph order)

            indexOf = numNewColors
            for i in range(0, numNewColors):
                if np.array_equal(color, newColors[i]):
                    indexOf = i   #i.e., indexOf points the index of the color class of node v
                    break

            if indexOf == numNewColors:  #i.e., detect a brand new color class
        ...
        newDistMatrix = np.zeros(shape = (numNewColors, numNewColors), dtype = np.int64)  #e.g., iter 1: #distinct degree
        #print(f'distM={distMatrix}')

        for i in range(numNewColors):
            for j in range(i + 1, numNewColors):
                #print(newColors[i], newColors[j], measDenom, distDenom)
                d = metric(newColors[i], newColors[j], distMatrix, measDenom, distDenom) #key - grokking
                newDistMatrix[i, j] = d
                newDistMatrix[j, i] = d
        #print(f'NEW_distM={newDistMatrix}')
...
def demoRun(G, H, description, additionalInformation, metric, iterationBound = math.inf, convIterationBound = math.inf):
    startTime = time.time()
